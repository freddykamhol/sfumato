...
test('CalDAV synchronisiert Termine bidirektional und archiviert Löschungen',async t=>{
  ...
  const profile=await fetch(`${url}/api/caldav.mobileconfig`,{headers:{Cookie:cookie}});assert.equal(profile.status,200);assert.match(profile.headers.get('content-type'),/application\/x-apple-aspen-config/);const profileText=await profile.text();assert.match(profileText,/<key>CalDAVHostName<\/key>/);assert.match(profileText,/<string>studio<\/string>/);const devicePassword=profileText.match(/<key>CalDAVPassword<\/key><string>([^<]+)<\/string>/)?.[1];assert.ok(devicePassword);const profileClient=await createDAVClient({serverUrl:`${url}/caldav/`,credentials:{username:'studio',password:devicePassword},authMethod:'Basic',defaultAccountType:'caldav'});assert.equal((await profileClient.fetchCalendars()).length,1)
  ...
  const stale=await fetch(`${url}/caldav/calendar/device-event-1.ics`,{method:'PUT',headers:{...headers,'Content-Type':'text/calendar','If-Match':etag},body:event(firstStart)});assert.equal(stale.status,412)
