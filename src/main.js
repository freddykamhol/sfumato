...
function initAdmin() {
  ...
  modal.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-send-proposals]')){
      ...
      if(requestEntry){button.disabled=true;button.textContent='Wird gesendet …';const batch={id:`PROP-${Date.now().toString(36).toUpperCase()}`,sentAt:new Date().toISOString(),duration,slots},subject=modal.querySelector('label input:not([type="datetime-local"])')?.value||`${requestReference(requestEntry)} · Terminvorschläge`,intro=modal.querySelector('textarea')?.value||'Hallo, hier sind drei mögliche Termine für dein Tattoo:',slotText=slots.map(slot=>new Intl.DateTimeFormat('de-DE',{dateStyle:'full',timeStyle:'short'}).format(new Date(slot))).map(value=>`• ${value} Uhr`).join('\n'),message=`${intro}\n\n${slotText}\n\nBitte teile uns in deiner Antwort mit, welcher Termin für dich passt.`;apiJson('/api/messages/send',{method:'POST',body:JSON.stringify({requestId:requestEntry.id,to:requestEntry.email,subject,text:message})}).then(email=>{requestEntry.emails=[...(requestEntry.emails||[]),email];return updateRequest(requestEntry,{estimatedHours:duration,proposals:[...(requestEntry.proposals||[]),batch],status:'In Klärung'})}).then(()=>{closeModal();content.innerHTML=requestDetailView(requestEntry)}).catch(error=>{button.disabled=false;button.textContent='Vorschläge per Nachricht senden →';let failure=modal.querySelector('.modal-form-error');if(!failure){failure=document.createElement('p');failure.className='modal-form-error';button.before(failure)}failure.textContent=error.message||'Terminvorschläge konnten nicht gesendet werden.'})}else closeModal()
  ...
  content.addEventListener('click', async e => {
    ...
    if(e.target.closest('[data-proposals]')){const params=new URLSearchParams(location.search),request=demoRequests.find(item=>item.id===params.get('anfrage')),sentSlots=(request?.proposals||[]).flatMap(batch=>batch.slots||[]).map(slot=>new Date(slot)).filter(date=>Number.isFinite(date.getTime())),searchAfter=sentSlots.length?new Date(Math.max(...sentSlots.map(date=>date.getTime()))):new Date(),duration=Math.max(.5,Number.parseFloat(request?.duration||request?.estimatedHours||4)||4);openSlotsModal(searchAfter,duration,encodeURIComponent(customerKey(request||{})),'proposal');generateAvailableSlots(duration,searchAfter).forEach((slot,index)=>{const input=modal.querySelectorAll('[data-slot-start]')[index];if(input)input.value=localDateTime(slot)});modal.querySelector('h2')?.insertAdjacentHTML('afterend',`<label>BETREFF<input value="${requestReference(request)} · Terminvorschläge"></label>`);return}
