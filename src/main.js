...
function customersView(){const customers=[...customerRecords().values()].sort((a,b)=>a.name.localeCompare(b.name,'de'));return `<div class="admin-title"><div><span class="admin-kicker">KARTEI</span><h2>Kunden</h2><p>Kontaktdaten und Projektverlauf aus Anfragen und Terminakten.</p></div><b class="customer-count">${customers.length}</b></div>${customers.length?`<div class="customer-filter"><label for="customer-search">KUNDEN DURCHSUCHEN</label><div><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"></circle><path d="m16 16 4 4"></path></svg><input id="customer-search" type="search" data-customer-search-input placeholder="Name, Kontakt, Stil, Körperstelle, Notizen …" autocomplete="off"><span data-customer-filter-count>${customers.length} Kunden</span></div></div>`:''}<div class="customer-list">${customers.length?customers.map(customer=>`<button data-customer="${encodeURIComponent(customer.key)}" data-customer-search="${encodeURIComponent(JSON.stringify({...customer,notes:customerNotes.filter(note=>note.customerKey===customer.key)}))}"><span class="customer-avatar">${customer.name.split(/\s+/).map(part=>part[0]).slice(0,2).join('').toUpperCase()}</span><div><b>${customer.name}</b><small>${customer.email||'Keine E-Mail'} · ${customer.phone||'Keine Telefonnummer'}</small></div><div><b>${customer.requests.length}</b><small>Anfragen</small></div><div><b>${customer.appointments.length}</b><small>Termine</small></div></button>`).join(''):'<div class="request-empty"><span>00</span><h3>Noch keine Kunden.</h3><p>Kunden werden automatisch aus Anfragen und Terminen übernommen.</p></div>'}<div class="customer-filter-empty" hidden><span>Keine Treffer</span><p>Prüfe den Suchbegriff oder versuche ein anderes Kundenmerkmal.</p></div></div>`}
