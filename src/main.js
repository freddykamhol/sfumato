...
function initAdmin() {
  ...
  content.addEventListener('click', e => {
    ...
    if(callContact){const customer=customerRecords().get(decodeURIComponent(callContact.dataset.callContact));const phone=normalizePhone(customer.phone);if(matchMedia('(max-width: 767px) and (pointer: coarse)').matches){location.href=`tel:${phone}`;return}const phoneBlocks=formatPhoneBlocks(phone).split(' ').map(block=>`<span>${block}</span>`).join('');openModal(`<button class="modal-close" data-close>×</button><div class="call-dialog-head"><span class="admin-kicker">KUNDENKONTAKT</span><h2>${customer.name}</h2><p>Direkt anrufen oder den QR-Code mit dem Smartphone scannen.</p></div><div class="call-modal"><div class="call-primary"><div class="call-icon"><svg viewBox="0 0 24 24"><path d="M7 3h3l1.5 5-2 1.5a15 15 0 0 0 5 5L16 12.5l5 1.5v3c0 2.2-1.8 4-4 4A14 14 0 0 1 3 7c0-2.2 1.8-4 4-4Z"/></svg></div><span>TELEFONNUMMER</span><strong>${phoneBlocks}</strong><a href="tel:${phone}"><span>Jetzt anrufen</span><b>→</b></a></div><div class="call-qr-card"><span>SCAN TO CALL</span><div class="call-qr-code"><div class="qr-loading">QR-Code wird erstellt …</div></div><small>Kamera öffnen, scannen<br>und direkt anrufen.</small></div></div>`);QRCode.toDataURL(`tel:${phone}`,{width:320,margin:1,color:{dark:'#11110f',light:'#ffffff'}}).then(url=>{const target=modal.querySelector('.call-qr-code');if(target)target.innerHTML=`<img src="${url}" alt="QR-Code zum Anrufen von ${formatPhoneBlocks(phone)}">`});return}
