...
function realAppointmentView(id) {
  ...
  const notes=customerNotes.filter(note=>note.customerKey===noteKey).sort((a,b)=>(rank[a.relevance]??3)-(rank[b.relevance]??3)||new Date(b.createdAt)-new Date(a.createdAt))
...
function initAdmin() {
  ...
  content.addEventListener('click', e => {
    ...
    if(reschedule){const item=appointments.find(entry=>entry.id===reschedule.dataset.rescheduleAppointment);if(item)openSlotsModal(item.start,Math.max(.5,(new Date(item.end)-new Date(item.start))/3600000),encodeURIComponent(customerKey(item)),'reschedule');return}
    if(e.target.closest('[data-proposals]')){const params=new URLSearchParams(location.search),request=demoRequests.find(item=>item.id===params.get('anfrage'));const duration=Math.max(.5,Number.parseFloat(request?.duration||request?.estimatedHours||4)||4);openSlotsModal(new Date(Date.now()+7*86400000),duration,encodeURIComponent(customerKey(request||{})),'proposal');return}
    ...
    if(row){
      ...
      content.innerHTML=`<div class="detail-top"><button class="detail-back">← Anfragen</button><div><button class="secondary-action" data-call-contact="${encodeURIComponent(customerKey(r))}" data-appointment-message="true">Kunde kontaktieren</button><button class="secondary-action" data-question>Rückfrage senden</button><button class="save-settings" data-proposals>Terminvorschläge erstellen</button></div></div>
