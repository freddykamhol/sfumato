...
function settingsView(){const path=location.pathname.replace(/\/+$/,'');const section=path.endsWith('/benutzer')?'users':path.endsWith('/kalender')?'calendar':'interfaces';const nav=`<nav class="settings-subnav" aria-label="Einstellungsbereiche"><a class="${section==='interfaces'?'active':''}" href="/admin/einstellungen/schnittstellen/"><span>01</span>Schnittstellen</a><a class="${section==='users'?'active':''}" href="/admin/einstellungen/benutzer/"><span>02</span>Benutzer</a><a class="${section==='calendar'?'active':''}" href="/admin/einstellungen/kalender/"><span>03</span>Kalender</a></nav>`;const head=`<div class="admin-title settings-title"><div><span class="admin-kicker">SYSTEM</span><h2>Einstellungen</h2><p>Studio, Zugänge und verbundene Dienste zentral verwalten.</p></div><button class="save-settings">Änderungen speichern</button></div>${nav}`;if(section==='users')return `${head}<div class="settings-section-head"><span class="admin-kicker">ZUGRIFF & ROLLEN</span><h3>Benutzer</h3><p>Personen mit Zugriff auf das Studio-Adminpanel.</p></div><div class="admin-users"><article><div class="avatar">TS</div><span><b>Studio Sfumato</b><small>Administrator · Vollzugriff</small></span><i>AKTIV</i><button>Bearbeiten</button></article></div><button class="secondary-action settings-add-user">＋ Benutzer einladen</button>`;if(section==='calendar')return `${head}<div class="settings-section-head"><span class="admin-kicker">VERFÜGBARKEIT</span><h3>Kalender</h3><p>Öffnungszeiten und Grundlage für automatische Terminvorschläge.</p></div><div class="settings-layout"><section class="integration-card calendar-connection"><div class="integration-icon">G</div><div><h3>Google Kalender</h3><p>Termine lesen und bestätigte Termine synchronisieren.</p></div><span class="connected">VERBUNDEN</span><button>Kalenderverbindung verwalten</button></section><section class="hours-card"><div class="settings-heading"><div><h3>Öffnungszeiten</h3><p>Basis für automatisch berechnete Terminvorschläge.</p></div><span>Europe/Berlin</span></div>${['Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag'].map((day,index)=>`<div class="hours-row"><label class="switch"><input type="checkbox" ${index!==0&&index!==5?'checked':''}><i></i></label><b>${day}</b><input type="time" value="10:00" ${index===0||index===5?'disabled':''}><span>bis</span><input type="time" value="18:00" ${index===0||index===5?'disabled':''}></div>`).join('')}<div class="hours-row"><label class="switch"><input type="checkbox"><i></i></label><b>Sonntag</b><span class="closed">Geschlossen</span></div></section></div>`;return `${head}<div class="settings-section-head"><span class="admin-kicker">VERBINDUNGEN</span><h3>Schnittstellen</h3><p>Externe Dienste für Kommunikation und Studio-Abläufe.</p></div><div class="settings-layout"><section class="integration-card"><div class="integration-icon">G</div><div><h3>Google Kalender</h3><p>Termine lesen, freie Zeiten prüfen und Termine eintragen.</p></div><span class="connected">AKTIV</span><button>Verbindung verwalten</button></section><section class="integration-card"><div class="integration-icon mail">✉</div><div><h3>SMTP E-Mail</h3><p>Rückfragen und Terminvorschläge direkt versenden.</p></div><span class="connected">AKTIV</span><button data-smtp>SMTP konfigurieren</button></section><section class="integration-card"><div class="integration-icon">↙</div><div><h3>E-Mail Eingang</h3><p>Antworten anhand der #Anfragenummer automatisch zuordnen.</p></div><span class="connected">BEREIT</span><button>Webhook konfigurieren</button></section></div>`}
...
function initAdmin() {
  ...
  modal.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-regenerate-slots]')){modal.querySelectorAll('[data-slot-start]').forEach((input,index)=>{const next=new Date(input.value);next.setDate(next.getDate()+21+(index*2));input.value=localDateTime(next)});return}
  ...
  content.addEventListener('click', async e => {
    ...
    if(reschedule){const item=appointments.find(entry=>entry.id===reschedule.dataset.rescheduleAppointment);if(item){openSlotsModal(item.start,Math.max(.5,(new Date(item.end)-new Date(item.start))/3600000),encodeURIComponent(customerKey(item)),'reschedule');const requestEntry=demoRequests.find(entry=>entry.id===item.requestId)||demoRequests.find(entry=>customerKey(entry)===customerKey(item));modal.querySelector('h2')?.insertAdjacentHTML('afterend',`<label>BETREFF<input value="${requestEntry?`${requestReference(requestEntry)} · `:''}Terminverschiebung"></label>`)}return}
    if(e.target.closest('[data-proposals]')){const params=new URLSearchParams(location.search),request=demoRequests.find(item=>item.id===params.get('anfrage'));const duration=Math.max(.5,Number.parseFloat(request?.duration||request?.estimatedHours||4)||4);openSlotsModal(new Date(Date.now()+7*86400000),duration,encodeURIComponent(customerKey(request||{})),'proposal');modal.querySelector('h2')?.insertAdjacentHTML('afterend',`<label>BETREFF<input value="${requestReference(request)} · Terminvorschläge"></label>`);return}
