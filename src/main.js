...
function initAdmin() {
  ...
  content.addEventListener('click', e => {
    ...
    if(callContact){const customer=customerRecords().get(decodeURIComponent(callContact.dataset.callContact));const phone=normalizePhone(customer.phone);if(matchMedia('(max-width: 767px) and (pointer: coarse)').matches){location.href=`tel:${phone}`;return}openModal(`<button class="modal-close" data-close>×</button><span class="admin-kicker">KUNDENKONTAKT</span><h2>Anrufen</h2><div class="call-modal"><div><span>TELEFONNUMMER</span><strong>${formatPhoneBlocks(phone)}</strong><a class="save-settings" href="tel:${phone}">Jetzt anrufen →</a></div><div class="call-qr"><div class="qr-loading">QR-Code wird erstellt …</div><small>Mit dem Handy scannen und direkt anrufen</small></div></div>`);QRCode.toDataURL(`tel:${phone}`,{width:260,margin:1,color:{dark:'#11110f',light:'#ffffff'}}).then(url=>{const target=modal.querySelector('.call-qr');if(target)target.innerHTML=`<img src="${url}" alt="QR-Code zum Anrufen von ${formatPhoneBlocks(phone)}"><small>Mit dem Handy scannen und direkt anrufen</small>`});return}
