...
function initAdmin() {
  ...
  modal.addEventListener('click',e=>{
    ...
    if(e.target.closest('[data-send-proposals]')){
      ...
      if(requestEntry){button.disabled=true;button.textContent='Wird gesendet …';const subject=modal.querySelector('label input:not([type="datetime-local"])')?.value||`${requestReference(requestEntry)} · Terminvorschläge`,message=modal.querySelector('textarea')?.value||'Hallo, hier sind drei mögliche Termine für dein Tattoo:';apiJson('/api/proposals/send',{method:'POST',body:JSON.stringify({requestId:requestEntry.id,subject,text:message,duration,slots})}).then(result=>{requestEntry.emails=[...(requestEntry.emails||[]),result.email];requestEntry.proposals=[...(requestEntry.proposals||[]),result.batch];requestEntry.status=result.status;requestEntry.estimatedHours=duration;closeModal();content.innerHTML=requestDetailView(requestEntry)}).catch(error=>{button.disabled=false;button.textContent='Vorschläge per Nachricht senden →';let failure=modal.querySelector('.modal-form-error');if(!failure){failure=document.createElement('p');failure.className='modal-form-error';button.before(failure)}failure.textContent=error.message||'Terminvorschläge konnten nicht gesendet werden.'})}else closeModal()
