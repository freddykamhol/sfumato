  ...
  return `
    ...
    <main id="top">
      ...
      <section id="termin" class="booking section-pad">
        ...
        <form id="booking-form" class="booking-form"><label>DEIN NAME<input required name="name" placeholder="Vor- und Nachname"></label><div class="form-row"><label>E-MAIL<input required type="email" name="email" placeholder="name@email.de"></label><label>TELEFON <small>OPTIONAL</small><input name="phone" placeholder="+49 ..."></label></div><div class="form-row"><label>STIL<select name="style"><option>Bitte auswählen</option><option>Fine Line</option><option>Blackwork</option><option>Floral / Organisch</option><option>Andere Richtung</option></select></label><label>KÖRPERSTELLE<input required name="placement" placeholder="z. B. Unterarm innen"></label></div><div class="form-row"><label>UNGEFÄHRE GRÖSSE <small>IN CM</small><input required name="size" inputmode="decimal" placeholder="z. B. 15 × 10 cm"></label><label>BERATUNG VORAB?<select name="consultation" id="consultation"><option value="no">Nicht nötig</option><option value="yes">Bitte erst ein Beratungsgespräch</option></select></label></div><div class="consultation-choice" hidden><label>ART DER BERATUNG<select name="consultationType" id="consultation-type"><option value="">Bitte auswählen</option><option value="studio">Persönlich im Studio</option><option value="phone">Telefonisch</option></select></label></div><label>ERZÄHL UNS VON DEINER IDEE<textarea required name="idea" rows="4" placeholder="Motiv, Bedeutung, Wünsche …"></textarea></label><label class="upload"><input type="file" name="reference" accept="image/*" multiple><span class="plus">+</span><span><b>Referenzen hinzufügen</b><small>JPG, PNG · max. 10 MB</small></span></label><label class="consent"><input required type="checkbox"><span></span><small>Ich stimme der Verarbeitung meiner Angaben zur Bearbeitung der Anfrage zu.</small></label><button class="button primary submit" type="submit">Anfrage senden ${arrow}</button><p class="form-message" role="status"></p></form>
      </section>
    </main>
