...
const apiJson=async(url,options={})=>{const response=await fetch(url,{headers:{'Content-Type':'application/json',Accept:'application/json',...(options.headers||{})},...options}),text=await response.text();let result=null;try{result=text?JSON.parse(text):null}catch{}if(!response.ok||!result){const error=new Error(result?.error||`Serverfehler (${response.status})`);if(result)Object.assign(error,result);throw error}return result}
...
const createAppointment = async payload => {
  try{return await apiJson('/api/appointments',{method:'POST',body:JSON.stringify(payload)})}catch(error){if(error.requiresConfirmation&&!payload.override&&confirm(`Achtung – der Termin ist nicht vollständig plausibel:\n\n${(error.warnings||[]).map(item=>`• ${item}`).join('\n')}\n\nTrotzdem verbindlich buchen und an den Kunden senden?`))return apiJson('/api/appointments',{method:'POST',body:JSON.stringify({...payload,override:true})});throw error}
}
...
function siteMarkup() {
  return `
    ...
    <footer><div class="footer-brand" data-protected-signature><img src="${signatureUrl}" alt="Signatur von Leon Zwezich" width="1536" height="1024" loading="lazy" decoding="async" draggable="false"></div><div><a href="#arbeiten">Arbeiten</a><a href="#about">Studio</a><a href="#termin">Booking</a><a href="/impressum/">Impressum</a><a href="/datenschutz/">Datenschutz</a><a href="/admin/">Admin</a></div><div class="social"><a href="https://www.instagram.com/tattoo_sfumato/?hl=de" target="_blank" rel="noopener noreferrer" aria-label="Tattoo Sfumato auf Instagram">${instagram}<span>@tattoo_sfumato</span></a><a href="https://www.instagram.com/leon_tats/" target="_blank" rel="noopener noreferrer" aria-label="Leon Tats auf Instagram">${instagram}<span>@leon_tats</span></a></div><p>© 2026 Tattoo Sfumato · Einbeck</p></footer><div class="toast" role="status"></div>`
}
...
const legalFooter=()=>`<footer><div class="footer-brand"><img src="${signatureUrl}" alt="Tattoo Sfumato" width="1536" height="1024" loading="lazy" decoding="async" draggable="false"></div><div><a href="/">Startseite</a><a href="/referenzen/">Referenzen</a><a href="/impressum/">Impressum</a><a href="/datenschutz/">Datenschutz</a></div><div class="social"><a href="mailto:info@tattoosfumato.de">info@tattoosfumato.de</a><a href="tel:+4915734408549">01573 4408549</a></div><p>© ${new Date().getFullYear()} Tattoo Sfumato · Einbeck</p></footer>`
...
function requestDetailView(r){
  const reference=requestReference(r),hours=Math.max(.5,Number(r.estimatedHours)||4),proposals=r.proposals||[],requestTimeline=r.timeline||[],mailTimeline=(r.emails||[]).filter(mail=>!requestTimeline.some(entry=>Math.abs(new Date(entry.createdAt)-new Date(mail.createdAt))<2000)).map(mail=>({title:mail.direction==='inbound'?'E-Mail vom Kunden':'E-Mail gesendet',text:mail.subject||'',message:mail.text||'',mailId:mail.id,from:mail.from||mail.to,createdAt:mail.createdAt})),timeline=[{title:'Anfrage eingegangen',text:'Anfrage über das Bookingformular erhalten.',createdAt:r.date},...requestTimeline,...mailTimeline].sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt))
  ...
  const linkedAppointment=appointments.find(item=>item.id===r.bookedAppointmentId)||appointments.find(item=>item.requestId===r.id&&item.type!=='consultation')
...
function initAdmin() {
  ...
  const enhanceAppointmentActions=()=>{const id=new URLSearchParams(location.search).get('termin'),item=appointments.find(entry=>entry.id===id),actions=content.querySelector('.detail-top>div');if(!item||!actions||actions.querySelector('[data-cancel-appointment]'))return;actions.insertAdjacentHTML('afterbegin',`${item.type==='consultation'&&item.status!=='Erledigt'?`<button class="secondary-action" data-complete-consultation="${item.id}">Beratung abschließen</button>`:''}<button class="secondary-action" data-cancel-appointment="${item.id}">Termin absagen</button>`)}
  const enhanceRequestConsultation=()=>{const id=new URLSearchParams(location.search).get('anfrage'),requestEntry=demoRequests.find(entry=>entry.id===id);if(!requestEntry?.consultation)return;const action=content.querySelector('[data-proposals]'),consultationAppointment=appointments.find(item=>item.id===requestEntry.consultationAppointmentId)||appointments.find(item=>item.requestId===requestEntry.id&&item.type==='consultation');if(consultationAppointment&&!content.querySelector('[data-consultation-link]'))content.querySelector('.request-workspace')?.insertAdjacentHTML('afterend',`<section class="linked-appointment" data-consultation-link><div><span class="admin-kicker">BERATUNGSTERMIN</span><i>${consultationAppointment.status||'GEBUCHT'}</i></div><a href="/admin/terminakten/?termin=${encodeURIComponent(consultationAppointment.id)}"><span><b>${new Intl.DateTimeFormat('de-DE',{dateStyle:'full'}).format(new Date(consultationAppointment.start))}</b><small>${new Intl.DateTimeFormat('de-DE',{timeStyle:'short'}).format(new Date(consultationAppointment.start))} Uhr · ${Math.round((new Date(consultationAppointment.end)-new Date(consultationAppointment.start))/60000)} Minuten</small></span><em>Beratung öffnen →</em></a></section>`);if(!action)return;if(!consultationAppointment){action.dataset.proposalKind='consultation';action.textContent='Beratungstermine erstellen'}else if(!requestEntry.consultationCompletedAt){action.disabled=true;action.textContent='Haupttermin nach abgeschlossener Beratung'}else{action.dataset.proposalKind='main';action.textContent='Terminvorschläge erstellen'}}
  const enhanceAdminViews=()=>{enhanceAppointmentActions();enhanceRequestConsultation()};new MutationObserver(enhanceAdminViews).observe(content,{childList:true,subtree:false});enhanceAdminViews()
  ...
  modal.addEventListener('click',e=>{
    ...
    if(contextProposals){closeModal();content.querySelector('[data-proposals]')?.click();return}
