...
function initAdmin() {
  ...
  content.addEventListener('click', async e => {
    ...
    if(cancelAppointment){const item=appointments.find(entry=>entry.id===cancelAppointment.dataset.cancelAppointment);if(!item||!confirm('Diesen Termin wirklich absagen? Der Kunde erhält sofort eine E-Mail und die Anfrage wird wieder als neu einsortiert.'))return;cancelAppointment.disabled=true;try{await updateAppointment(item,{status:'Storniert',notifyCustomer:true});const requestEntry=demoRequests.find(entry=>entry.id===item.requestId);if(requestEntry){requestEntry.status='Neu';delete requestEntry.readAt;requestEntry.activityAt=new Date().toISOString();delete requestEntry.bookedAppointmentId}content.innerHTML=realAppointmentView(item.id)}catch(error){cancelAppointment.disabled=false;alert(error.message)}return}
