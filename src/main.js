...
const ruleRow=(rule={},index=Date.now())=>`<div class="calendar-rule" data-rule><select name="rule-type"><option value="weekday" ${rule.type==='weekday'?'selected':''}>Wochentag sperren</option><option value="time" ${rule.type==='time'?'selected':''}>Zeitfenster sperren</option><option value="duration" ${rule.type==='duration'?'selected':''}>Maximale Dauer</option><option value="lead" ${rule.type==='lead'?'selected':''}>Mindestvorlauf in Tagen</option></select><select name="rule-operator"><option value="is">ist</option><option value="after" ${rule.operator==='after'?'selected':''}>nach</option><option value="before" ${rule.operator==='before'?'selected':''}>vor</option><option value="max" ${rule.operator==='max'?'selected':''}>maximal</option></select><input name="rule-value" value="${rule.value||''}" placeholder="Wert"><input name="rule-label" value="${rule.label||''}" placeholder="Bezeichnung"><label class="switch"><input type="checkbox" name="rule-enabled" ${rule.enabled!==false?'checked':''}><i></i></label><button type="button" data-remove-rule>×</button></div>`
