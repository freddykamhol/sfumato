...
const ruleRow=(rule={},index=Date.now())=>{const type=rule.type||'weekday',days=['Sonntag','Montag','Dienstag','Mittwoch','Donnerstag','Freitag','Samstag'],value=rule.value??(type==='weekday'?'0':type==='time'?'18:00':type==='duration'?'6':'3'),operator=rule.operator||(type==='time'?'after':type==='duration'?'max':'is'),condition=type==='weekday'?`<span>Termine an</span><select name="rule-value" aria-label="Wochentag">${days.map((day,dayIndex)=>`<option value="${dayIndex}" ${String(value)===String(dayIndex)?'selected':''}>${day}</option>`).join('')}</select><input type="hidden" name="rule-operator" value="is"><span>nicht vorschlagen</span>`:type==='time'?`<span>Termine</span><select name="rule-operator" aria-label="Zeitrichtung"><option value="after" ${operator==='after'?'selected':''}>ab</option><option value="before" ${operator==='before'?'selected':''}>vor</option></select><input type="time" name="rule-value" value="${value}"><span>nicht vorschlagen</span>`:type==='duration'?`<span>Termine dürfen höchstens</span><input type="number" min="0.5" max="24" step="0.5" name="rule-value" value="${value}"><input type="hidden" name="rule-operator" value="max"><span>Stunden dauern</span>`:`<span>Termine frühestens</span><input type="number" min="0" max="365" step="1" name="rule-value" value="${value}"><input type="hidden" name="rule-operator" value="is"><span>Tage im Voraus vorschlagen</span>`;return `<article class="calendar-rule" data-rule><header><span class="rule-number">REGEL</span><label class="switch" title="Regel aktivieren"><input type="checkbox" name="rule-enabled" ${rule.enabled!==false?'checked':''}><i></i></label><button type="button" data-remove-rule aria-label="Regel löschen">×</button></header><div class="rule-sentence"><strong>Wenn</strong><select name="rule-type" data-rule-type><option value="weekday" ${type==='weekday'?'selected':''}>ein bestimmter Wochentag ist</option><option value="time" ${type==='time'?'selected':''}>eine Uhrzeit erreicht ist</option><option value="duration" ${type==='duration'?'selected':''}>die Termindauer geplant wird</option><option value="lead" ${type==='lead'?'selected':''}>ein Termin vorgeschlagen wird</option></select><strong>dann</strong>${condition}</div><label class="rule-label">INTERNE BEZEICHNUNG <input name="rule-label" value="${rule.label||''}" placeholder="Optional, z. B. Sonntags geschlossen"></label></article>`}
...
function initAdmin() {
  ...
  content.addEventListener('submit',async e=>{const form=e.target.closest('[data-calendar-settings-form]');if(!form)return;e.preventDefault();const calendar={...studioSettings.calendar,beforeMinutes:Number(form.elements.beforeMinutes.value)||0,afterMinutes:Number(form.elements.afterMinutes.value)||0,hours:Array.from({length:7},(_,index)=>({enabled:form.querySelector(`[name="hours.${index}.enabled"]`).checked,start:form.querySelector(`[name="hours.${index}.start"]`).value,end:form.querySelector(`[name="hours.${index}.end"]`).value})),rules:[...form.querySelectorAll('[data-rule]')].map(row=>({type:row.querySelector('[name="rule-type"]').value,operator:row.querySelector('[name="rule-operator"]').value,value:row.querySelector('[name="rule-value"]').value,label:row.querySelector('[name="rule-label"]').value,enabled:row.querySelector('[name="rule-enabled"]').checked}))};await saveStudioSettings({...studioSettings,calendar});form.querySelector('[type="submit"]').textContent='Gespeichert ✓'})
  content.addEventListener('change',e=>{const typeSelect=e.target.closest('[data-rule-type]');if(!typeSelect)return;const row=typeSelect.closest('[data-rule]'),label=row.querySelector('[name="rule-label"]').value,enabled=row.querySelector('[name="rule-enabled"]').checked;row.outerHTML=ruleRow({type:typeSelect.value,label,enabled})})
  content.addEventListener('submit',async e=>{
  ...
  content.addEventListener('click', async e => {
    ...
    if(newUser||editUser){const user=editUser?adminUsers.find(item=>item.id===editUser.dataset.editUser):null;openModal(`<button class="modal-close" data-close>×</button><span class="admin-kicker">BENUTZER</span><h2>${user?'Benutzer bearbeiten':'Benutzer anlegen'}</h2><form data-user-form ${user?`data-user-id="${user.id}"`:''}><label>NAME<input required name="name" value="${user?.name||''}"></label><label>E-MAIL<input required type="email" name="email" value="${user?.email||''}"></label><label>ROLLE<select name="role"><option ${user?.role==='Administrator'?'selected':''}>Administrator</option><option ${user?.role==='Studio'?'selected':''}>Studio</option><option ${user?.role==='Lesen'?'selected':''}>Lesen</option></select></label><label>PASSWORT<input ${user?'':'required'} type="password" name="password" placeholder="${user?'Leer lassen, um es beizubehalten':'Sicheres Passwort'}"></label><div class="reference-switches"><label><input type="checkbox" name="active" ${user?.active!==false?'checked':''}><span></span>Benutzer aktiv</label></div><p class="modal-form-error"></p><button class="save-settings modal-send" type="submit">Benutzer speichern →</button></form>`);return}
