...
const sendRequestMail=async({requestEntry,to,subject,text,attachments=[]})=>{const settings=await readSettings(),config=settings.integrations?.smtp,reference=requestReference(requestEntry),safeSubject=String(subject||'Nachricht zu deiner Tattoo-Anfrage').replace(/[\r\n]+/g,' ').trim(),subjectWithReference=safeSubject.includes(reference)?safeSubject:`${reference} · ${safeSubject}`,body=`${String(text||'').trim()}\n\n---\n${mailReplyNotice(reference)}`,transporter=smtpTransport(config),result=await transporter.sendMail({from:config.from||config.user,replyTo:config.from||config.user,to,subject:subjectWithReference,text:body,attachments:mailAttachments(attachments)});return{messageId:result.messageId,subject:subjectWithReference,text:body}}
