...
const notifyTelegram=async(text,buttonUrl='')=>{try{const settings=await readSettings(),config=settings.integrations?.telegram;if(!config?.enabled||!config.token||!config.chatId)return;const payload={chat_id:config.chatId,text};if(buttonUrl)payload.reply_markup={inline_keyboard:[[{text:'Anfrage im Adminpanel öffnen',url:buttonUrl}]]};const telegramResponse=await fetch(`https://api.telegram.org/bot${config.token}/sendMessage`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)});if(!telegramResponse.ok)throw new Error(`Telegram API ${telegramResponse.status}`)}catch(error){console.error('[telegram]',error.message)}}
  ...
  if (pathname === '/api/requests' && request.method === 'POST') {
    ...
    request.on('end', async () => {
      ...
      try {
        ...
        notifyTelegram(`NEUE ANFRAGE\nNummer: ${entry.reference}\nName: ${entry.name}\nTelefon: ${entry.phone||'Nicht angegeben'}\nE-Mail: ${entry.email}\nStil: ${entry.style}\nGröße: ${entry.size}\nStelle: ${entry.placement}`,adminUrl)
