...
const ensureDefaultAdmin=async()=>{const users=await readUsers(),settings=await readSettings();let changed=false;for(const user of users){if(user.password&&!user.password.startsWith('scrypt:')){user.password=hashPassword(user.password);changed=true}if(!user.username){user.username=user.email?.split('@')[0]||user.name?.toLowerCase().replace(/\s+/g,'.');changed=true}}let initialAdmin=users.find(user=>user.id==='USR-ADMIN'||user.username?.toLowerCase()==='admin'&&user.initial);if(!initialAdmin&&!settings.auth?.defaultAdminRemoved){initialAdmin={id:'USR-ADMIN',username:'admin',name:'Administrator',email:'admin@local',role:'Administrator',active:true,password:hashPassword('admin123'),createdAt:new Date().toISOString(),initial:true};users.push(initialAdmin);changed=true}else if(initialAdmin?.initial&&!verifyPassword('admin123',initialAdmin.password)){initialAdmin.password=hashPassword('admin123');initialAdmin.active=true;changed=true}if(changed)await saveUsers(users)}
