...
const ensureDefaultAdmin=async()=>{const users=await readUsers();let changed=false;for(const user of users){if(user.password&&!user.password.startsWith('scrypt:')){user.password=hashPassword(user.password);changed=true}user.username=user.username||user.email?.split('@')[0]||user.name?.toLowerCase().replace(/\s+/g,'.')}if(!users.length){users.push({id:'USR-ADMIN',username:'admin',name:'Administrator',email:'admin@local',role:'Administrator',active:true,password:hashPassword('admin123'),createdAt:new Date().toISOString(),initial:true});changed=true}if(changed)await saveUsers(users)}
...
createServer(async (request, response) => {
  ...
  if(pathname==='/admin/login'&&request.method==='POST'){let body='';request.on('data',chunk=>{if(body.length<10000)body+=chunk});request.on('end',async()=>{const data=new URLSearchParams(body),identity=cleanText(data.get('username'),180).toLowerCase(),password=data.get('password')||'',users=await readUsers(),user=users.find(item=>item.active&&(item.username?.toLowerCase()===identity||item.email?.toLowerCase()===identity));if(!user||!verifyPassword(password,user.password)){response.writeHead(401,{'Content-Type':'text/html; charset=utf-8','Cache-Control':'no-store'});response.end(adminLoginPage(true));return}response.writeHead(303,{Location:'/admin/','Set-Cookie':`${adminCookieName}=${createAdminToken(user)}; HttpOnly; SameSite=Strict; Path=/; Max-Age=43200${secureCookie}`,'Cache-Control':'no-store'});response.end()});return}
